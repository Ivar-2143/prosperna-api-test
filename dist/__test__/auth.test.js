...
describe("Login Auth Testing", () => {
    ...
    beforeAll(async () => {
        try {
            ...
            server = app_1.default.listen(port, () => console.log("now listening to server on port " + port));
    ...
    it("it Tests POST /login given a valid credentials, it should log in", async () => {
        const res = await axios_1.default.post(`${BASE_URL}/login`, data);
    ...
    it("it Tests POST /login given an invalid credentials it should not log in", async () => {
        await axios_1.default
            ...
            expect(err.response.data.message).toBe("Email or password is incorrect");
        });
    });
