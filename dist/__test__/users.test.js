...
describe("User Endpoints Testing", () => {
    ...
    it("it Tests POST - /users/register given a valid parameters, it should return successfully registered.", async () => {
        ...
        try {
            const res = await axios_1.default.post(`${BASE_URL}/users/register`, data);
    ...
    it("it Tests POST - /users/register given a invalid email, it should return invalid email message", async () => {
        ...
        axios_1.default.post(`${BASE_URL}/users/register`, data).catch((err) => {
    ...
    it("it Tests POST - /users/register given a blank password, it should return required password message", async () => {
        ...
        axios_1.default.post(`${BASE_URL}/users/register`, data).catch((err) => {
            expect(err.response.statusText).toBe("Bad Request");
            expect(err.response.data.errors["password"]).toBe("String must contain at least 8 character(s)");
        });
    });
    it("it Tests POST - /users/register given a unmatching password, it should return that password doesn't match", async () => {
        ...
        axios_1.default.post(`${BASE_URL}/users/register`, data).catch((err) => {
            expect(err.response.statusText).toBe("Bad Request");
            expect(err.response.data.errors["password_confirmation"]).toBe("Password does not match");
        });
    });
    it("Tests GET - /users It should return the List of Users", async () => {
        ...
        await axios_1.default.post(`${BASE_URL}/users/register`, dummy_data);
    ...
    it("Tests GET - /users/:id given that id is valid, It should return the user info", async () => {
        const res = await axios_1.default.get(`${BASE_URL}/users`);
        const singleUser = await axios_1.default.get(`${BASE_URL}/users/${res.data.users[0]._id}`);
    ...
    it("Tests GET - /users/:id given that id doesn't exist, It should return error not found", async () => {
        axios_1.default.get(`${BASE_URL}/users/${new mongodb_1.ObjectId()}`).catch((err) => {
    ...
    it("Tests PUT - /users/:id given that data is valid, It should update the user data", async () => {
        ...
        const login = await axios_1.default.post(`${BASE_URL}/login`, data);
        const headers = {
            Authorization: `Bearer ${login.data.token}`,
        };
        const loggedInUser = await axios_1.default.get(`${BASE_URL}/login`, {
            headers: headers,
        });
        const update = await axios_1.default.put(`${BASE_URL}/users/${loggedInUser.data.user._id}`, updated_data, {
            headers: headers,
        });
    ...
    it("Tests PUT - /users/:id given that the account is not yours, It should return an forbidden error", async () => {
        ...
        const login = await axios_1.default.post(`${BASE_URL}/login`, {
            email: users.data.users[0].email,
            password: "admin12345",
        });
        axios_1.default
            .put(`${BASE_URL}/users/${users.data.users[1]._id}`, { email: "new_email@gmail.com", password: "admin12345" }, { headers: { Authorization: `Bearer ${login.data.token}` } })
            .catch((err) => {
            expect(err.response.status).toBe(403);
            expect(err.response.data.message).toBe("You are not allowed to update this account");
        });
    });
    it("Tests PUT - /users/:id given that email is invalid, It should not update the user data", async () => {
        ...
        const loggedInUser = await axios_1.default.get(`${BASE_URL}/login`, { headers });
        axios_1.default
            .put(`${BASE_URL}/users/${loggedInUser.data.user._id}`, updated_data, {
    ...
    it("Test DELETE - /users/:id given an invalid ID, it should delete the owner's account", async () => {
        ...
        axios_1.default
            .delete(`${BASE_URL}/users/${new mongodb_1.ObjectId()}`, {
            headers: {
                Authorization: `Bearer ${login.data.token}`,
            },
        })
            .catch((err) => {
            expect(err.response.status).toBe(403);
            expect(err.response.data.message).toBe("You are not allowed to delete this account");
        });
    });
    it("Test DELETE - /users/:id given a valid ID, it should delete the owner's account", async () => {
        ...
        const removed_account = await axios_1.default.delete(`${BASE_URL}/users/${users.data.users[0]._id}`, {
            headers: {
                Authorization: `Bearer ${login.data.token}`,
            },
        });
